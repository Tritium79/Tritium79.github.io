...
| `@media (max-aspect-ratio: 0.9/1)` | 移动端响应式 | 以宽高比而非像素宽度作为断点 |
...
- **断点**：`min-aspect-ratio: 2.1/1`（宽高比大于 2.1:1 时触发）
...
3. **移动端同步**：在 `max-aspect-ratio: 0.9/1` 媒体查询中，任何对桌面布局的结构性改动都需要检查移动端对应规则。
